...
public class RSAUtil {
    ...
    public static Map<String, String> genKeyPair(int keySize) {
        ...
        try {
            ...
            e.printStackTrace();
            System.out.println("生成公私钥信息失败：" + e.getMessage());
        }
        return keyMap;
    }
    ...
    private static byte[] encryptByPublicKey(byte[] data, String publicKey, int length) {
        ...
        try {
            ...
            e.printStackTrace();
            System.out.println("公钥分段加密失败：" + e.getMessage());
        } finally {
            try {
                out.close();
            } catch (IOException e) {
...
/** */
    ...
    private static byte[] decryptByPrivateKey(byte[] data, String privateKey, int length) {
        ...
        try {
            ...
            e.printStackTrace();
            System.out.println("私钥解密失败：" + e.getMessage());
        } finally {
            try {
                out.close();
            } catch (IOException e) {
     ...
     * @return
     */
    public static String decryptByPrivateKey(String data, String rsaKey) {
        //log.info("私钥解密内容："+new String(decryptByPrivateKey(decryptBASE64(data.replaceAll("@", "+")), getRsaKey(), MAX_DECRYPT_BLOCK)));
        return new String(decryptByPrivateKey(decryptBASE64(data.replaceAll("@", "+")), rsaKey, MAX_DECRYPT_BLOCK));
    }
